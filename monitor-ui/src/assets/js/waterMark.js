export function watermark(settings) {
  ...
  // 获取页面最大宽度
  const p_width = Math.max(document.body.scrollWidth,document.body.clientWidth)
  const cutWidth = p_width*0.0150
  const page_width=p_width-cutWidth
  // 获取页面最大高度
  // let page_height = Math.max(document.body.scrollHeight,document.body.clientHeight)+450;
  const page_height = Math.max(document.body.scrollHeight,document.body.clientHeight)- 100
