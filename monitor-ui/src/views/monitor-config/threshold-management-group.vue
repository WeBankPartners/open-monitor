    ...
    <section v-if="showManagement" style="margin-top: 16px;">
      ...
      <PageTable :pageConfig="pageConfig">
        ...
        <section class="receiver-config">
          <!-- <section :key="tableIndex + 'a'" v-if="showReceiver" style="margin: 16px 0">
            <span class="receiver-header">{{$t('button.receivers')}}:</span>
            <Tag 
              v-for="(receiver, receiverIdex) in groupNotify" 
              type="border" 
              :key="receiverIdex" 
              color="primary">
              {{receiver.alarm_action}}
            </Tag>
            
          </section> -->
          <div style="margin: 16px 0">
            <h5 style="display:inline-block">{{$t('button.receiversConfiguration')}}:</h5>
            <button @click="showReceiver = !showReceiver" class="btn btn-small btn-cancel-f">{{$t('button.edit')}}</button>
            <div class="receiver-config-set" style="margin: 8px 0">
            <template>
                <div style="margin: 4px 0px;padding:8px 12px;width:640px">
                  <template v-for="(item, index) in groupNotify">
                    <p :key="index + 'S'">
                      <Button
                        ...
                        icon="md-close"
                      ></Button>
                      <Select v-model="item.alarm_action" :disabled="!showReceiver" style="width: 100px" :placeholder="$t('alarm_action')">
                        <Option v-for="type in ['firing', 'ok']" :key="type" :value="type">{{type}}</Option>
                      </Select>
                      <Select v-model="item.proc_callback_key" :disabled="!showReceiver" style="width: 160px" :placeholder="$t('proc_callback_key')">
                        <Option v-for="(flow, flowIndex) in flows" :key="flowIndex" :value="flow.option_value">{{flow.option_text}}</Option>
                      </Select>
                      <Select v-model="item.notify_roles" :disabled="!showReceiver" style="width: 320px" multiple filterable>
        ...
        <div style="margin: 4px 0px;padding:8px 12px;border:1px solid #dcdee2;border-radius:4px;">
          <template v-for="(item, index) in modelConfig.addRow.notify">
            <p :key="index + 'S'">
              ...
              <Select v-model="item.notify_roles" style="width: 320px" multiple filterable>
                <Option v-for="item in allRole" :value="item.name" :key="item.value">{{ item.name }}</Option>
              </Select>
            </p>
          </template>
...
export default {
  ...
  methods: {
    ...
    getWorkFlow () {
      this.$root.$httpRequestEntrance.httpRequestEntrance('GET', '/monitor/api/v1/alarm/org/callback/get?guid=sys01', '', (responseData) => {
        this.flows = responseData.firing_callback
      })
    },
    getAllRole () {
    ...
    getNotifyDetail () {
      const api = `/monitor/api/v2/alarm/endpoint_group/${this.targrtId}/notify/list`
      this.$root.$httpRequestEntrance.httpRequestEntrance('GET', api, '', (responseData) => {
        this.groupNotify = responseData
      })
    },
  },
  components: {},
