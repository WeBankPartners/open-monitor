            ...
            <template v-if="tableItem.groupNotify.length > 0">
              <div style="margin: 4px 0px;padding:8px 12px;">
                <template v-for="(item, index) in tableItem.groupNotify">
                  <p :key="index + 'S'">
                    <Button
                      @click="deleteItem('group', index, tableItem)"
                      size="small"
...
export default {
  ...
  methods: {
    ...
    deleteItem(type, index, tableItem) {
      if (type === 'group') {
        tableItem.groupNotify.splice(index, 1)
      } else {
    ...
    async editF (rowData) {
      this.selectedData = rowData
      const api = `/monitor/api/v2/monitor/metric/list?monitorType=${rowData.monitor_type}&serviceGroup=${this.targrtId}`
    ...
    async add (tableItem) {
      ...
      const api = `/monitor/api/v2/monitor/metric/list?monitorType=${tableItem.monitor_type}&serviceGroup=${this.targrtId}`
      this.$root.$httpRequestEntrance.httpRequestEntrance('GET', api, '', (responseData) => {
        this.modelConfig.metricList = responseData
      })
      // await this.getAllRole()
    ...
    getDetail (targrtId) {
      ...
      const api = '/monitor/api/v2/alarm/strategy/list/service/' + targrtId
      this.totalPageConfig = []
      this.$root.$httpRequestEntrance.httpRequestEntrance('GET', api, '', (responseData) => {
        this.showManagement = true
        responseData.forEach((item)=>{
          let config = this.$root.$validate.deepCopy(this.pageConfig.table)
          config.tableData = item.strategy.map(s => {
