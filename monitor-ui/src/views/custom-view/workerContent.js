...
export const initPanalsWebWorker = dataObj => {
  ...
  const filterLayoutData = (layoutData, activeGroup, chartLayoutType = 'customize') => {
    if (activeGroup === 'ALL') {
      layoutData = calculateLayout(layoutData, chartLayoutType)
    } else {
      layoutData = layoutData.filter(d => d.group === activeGroup)
      ...
      chartLayoutType = initLayoutTypeByWidth(layoutData)
    ...
    chartLayoutType = confirmLayoutType(layoutData)
    return {
      layoutData,
      chartLayoutType
    }
  ...
  if (!tempLayoutData.length || actionType === 'init') {
    ...
    tmpArr.forEach(tmpItem => {
      ...
      if (item) {
        console.error(item, tmpItem, '776688')
