                    ...
                    <ul>
                      ...
                      <template v-for="(filter,j) in filterListsBySearch">
                        <li class="filters-li" @click="filterTable(item, tableEleIndex, filter)" :key='j'
                            :class="{active: getColumnFilterActiveno(tableEleIndex)._activeKey === filter.value}">{{filter.label}}</li>
                      </template>
                    </ul>
                  </div>
                </Poptip>
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      // if (this.$root.$validate.isEmpty(this.$store.state[this.$router.history.current.name])) {
    ...
    methods: {
      ...
      filterMoreBtn (value) {
        ...
        if (this.$root.$validate.isEmpty(this.table.filterMoreBtn)) {
          ...
          for (let show_btn of moreBtnGroup_show) {
            for (let index in this.moreBtnGroup) {
      ...
      filterIcon (columnNo) {
        ...
        for (let item in this.pageConfig.table.tableEle) {
      ...
      // 多选重置参数
      resetMultiFilter: function(ele, i) {
        this.pageConfig.table.tableEle[i]._isFilter = 'N'
      ...
      renderValue: function (item, config) {
        ...
        let expr=config.value || config.val
        let item_tmp = item
      ...
      doCopy: function (no, eleConfig) {
        let clipboardMsg = this.dataProcess(no, eleConfig)
      ...
      //无数据时系新增跳转
      goToConfig(tip){
        this.$router.push({name: tip.url,params:tip})
      },
      //是否显示省略号
      ellipsis(value,val){
        let valuelength = this.renderValue(value, val)?this.renderValue(value, val).length:0
        let maxlength = val.overflow?val.overflow:20 //通过配置overflow自定义显示字数
