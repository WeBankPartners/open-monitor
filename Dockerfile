...
# 同时创建空的原始目录供 PV 挂载
RUN mkdir -p $BASE_HOME $PROMETHEUS_TMP $PROMETHEUS_TMP/rules $PROMETHEUS_TMP/token $PROMETHEUS_TMP/custom_scrape \
...
# 基础镜像 v1.4 中已安装 sudo 并配置 app 用户无密码 sudo 权限
# 应用镜像新增的文件和目录需要设置为 app:apps，以便 app 用户可以正常访问
...
# 容器以 app 用户启动，start.sh 中使用 sudo 提权执行需要 root 权限的操作
USER app
